    ...
    firebase
      ...
      .then((snapshot) => {
        if (snapshot.exists) {
          ...
          console.log('publish data',PublisherType)
        } else {
...
//     const snapshot = await productRef.orderBy("id").limit(3).get();

//     console.log('snapshot...',snapshot._snapshot)
  ...
  return (
    <SafeAreaView style={styles.commanStyle}>
      ...
      <ScrollView>
          ...
          <Button title='click' onPress={()=>navigation.navigate('Object_scanner')}/>
        <View
          style={{
            flex: 1,
